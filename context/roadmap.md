...
**Phase 3 chunk 3.1 shipped.** Panel entity + R17 lifecycle invariants in place: `Document.panels: Record<PanelId, Panel>`, addPanel/removePanel/updatePanel commands, dispatch-rejection of operations that would open a paneled path (closed→open, role swap, nodeIds<3) or remove a paneled path without first removing its Panel. Pen tool's close-by-snap and cross-panel-join close-on-loop both batch addPanel into the close. deletePathsCommand and deleteNodesCommand cascade-remove Panels. JSON I/O round-trips panels with validation. 204 → 233 tests green.
...
Next: **Phase 3 chunk 3.2 — Seam edges** (typed edge ranges on panels, seam-pair links). Standing assignment from 2026-04-23 (find one non-Justin hobby sewist) still pending.
