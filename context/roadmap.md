...
**Phase 3 chunks 3.1 + 3.2 shipped.** Panel entity + R17 lifecycle in place; first annotation glyph (notches) live on closed panels with single/double ticks perpendicular to the edge. Cascades wired: deleteNodes drops notches on broken edges; insertNodeOnSegment re-pegs notches by remapping t (preserves visual position for line and cubic edges). 204 → 254 tests green.
...
Next: **Phase 3 chunk 3.3 — Darts** (port sketch's 3-phase dart tool + `dartExtension.ts` SA-cutline math). Standing assignment from 2026-04-23 (find one non-Justin hobby sewist) still pending.
...
- [ ] **3.3 Darts** — port sketch's 3-phase dart tool + `dartExtension.ts` SA-cutline math; dart-on-edge interaction with edge consumption (was 3.4)
