...
**Phase 3 chunks 3.1–3.5 + 3.7 (knife) shipped.** Panel entity + R17 lifecycle, notches, darts, landmark labels, ease/gather notations, and the slash-and-pivot knife instrument all live. Cascade-on-edge-break + re-peg-on-edge-subdivide pattern proven across notches and darts; ease/gather adds a second-tier cascade (notch-removal cascades any ease/gather pointing at it). Knife rides the same self-intersection elastic-clamp pattern as node drag. 204 → 324 tests green.
...
Next: **Phase 3 chunk 3.6 — Instruments, passive** (rulers + protractor first; french curves with trace-to-cubic-chain last) — and the deferred seam edges / seam-pair validator (3.8 / 3.9) once the feedback cycle with Justin lands. Standing assignment from 2026-04-23 (find one non-Justin hobby sewist) still pending. Knife's pending pre-code assignment — sketch one paper slash-and-pivot recipe — also still pending; surface it when reproducing a real recipe with the live tool.
