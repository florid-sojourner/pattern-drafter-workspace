...
**Phase 3 chunks 3.1 + 3.2 + 3.3 shipped.** Panel entity + R17 lifecycle in place; notches and darts both live as panel annotations. Darts ported faithfully from the sketch (3-phase click flow, dartExtension.ts SA-cutline math). Cascade-on-edge-break + re-peg-on-edge-subdivide is a shared pattern across all annotations. 204 → 275 tests green.
...
Next: **Phase 3 chunk 3.4 — Landmark labels** (text labels on guide landmark dots; cheap, orthogonal to panel work). Standing assignment from 2026-04-23 (find one non-Justin hobby sewist) still pending.
